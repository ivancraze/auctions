...
describe('AuctionBetPage', () => {
  ...
  /** Проверяет общую server-ошибку и повторную отправку после снятия submit-lock. */
  it('allows retrying a bet after a temporary server error', async () => {
    const mutationSpy = vi.fn()
    server.use(
      http.post('/api/v1/auctions/:auctionUuid/bets', () => {
        mutationSpy()

        if (mutationSpy.mock.calls.length === 1) {
    ...
    expect(await screen.findByText('Повторите попытку.')).toBeInTheDocument()
    expect(useToastStore.getState().toast).toMatchObject({
      kind: 'error',
      text: 'Ставка не была принята',
    })
    ...
    expect(
      await screen.findByText('Ставка успешно принята'),
    ).toBeInTheDocument()
